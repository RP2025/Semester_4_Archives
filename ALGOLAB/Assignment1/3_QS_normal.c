...
void getArray(double *arr, int n)
{
    ...
    FILE *fin = fopen("DataSet_ND.txt", "r");
...
int main()
{
    ...
    FILE *fout = fopen("QS_Normal_comparisons.txt", "w");
    ...
    for (int i = 0; i < power; i++)
    {
        ...
        float constant = avg_count / (n * log(n));
        //printf("\nN: %d, Avg time: %0.4f, Avg count: %ld\n", n, avg_time, avg_count);
        fprintf(fout, "%d, %0.4f, %ld, %f\n", n, avg_time, avg_count, constant);
        n *= 2;
    }
    return 0;
}